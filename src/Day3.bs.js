// Generated by BUCKLESCRIPT VERSION 5.0.6, PLEASE EDIT WITH CARE
'use strict';

var $$Array = require("bs-platform/lib/js/array.js");
var Block = require("bs-platform/lib/js/block.js");
var Curry = require("bs-platform/lib/js/curry.js");
var Printf = require("bs-platform/lib/js/printf.js");
var Caml_int32 = require("bs-platform/lib/js/caml_int32.js");
var Pervasives = require("bs-platform/lib/js/pervasives.js");
var Caml_format = require("bs-platform/lib/js/caml_format.js");
var Caml_option = require("bs-platform/lib/js/caml_option.js");
var Belt_MapString = require("bs-platform/lib/js/belt_MapString.js");
var Caml_exceptions = require("bs-platform/lib/js/caml_exceptions.js");

var ParseException = Caml_exceptions.create("Day3-AdventOfCode2019.ParseException");

var input = $$Array.map((function (param) {
        return param.split(",");
      }), "R1009,D335,L942,D733,L398,U204,L521,D347,L720,U586,R708,D746,L292,U416,L824,U20,R359,D828,R716,U895,L498,D671,L325,D68,L667,U134,L435,D44,R801,U654,R188,U542,L785,D318,L806,U602,L465,U239,R21,U571,R653,U436,L52,U380,R446,D960,R598,U590,L47,U972,L565,D281,R790,U493,R864,D396,R652,D775,L939,D284,R554,U629,L842,D837,R554,D795,R880,D301,R948,U974,L10,D898,R588,D743,L334,U59,L413,U511,L132,U771,R628,D805,R465,D561,R18,D169,L580,D99,L508,U964,L870,D230,L472,U897,L85,U306,L103,U322,L637,U464,R129,D514,R454,U479,R801,U18,R929,U181,L113,D770,L173,D124,L122,U481,L666,D942,L534,U608,R90,U576,L641,U249,L857,U197,R783,D92,L938,D192,L698,D862,R995,U12,R766,D323,R934,U315,R956,D234,R983,D246,L153,U26,L779,D628,R174,D385,L758,D486,R132,U414,R915,D511,L152,D309,L708,D755,L679,D166,L699,U734,R55,D224,L582,U798,L348,U219,L304,U621,L788,D538,R781,D509,R486,U581,R759,D892,R16,D552,L82,D618,L309,D610,L645,U146,L328,U569,L307,D385,L249,D231,R928,U681,R384,D337,R715,D798,L788,D604,R517,U766,R368,U430,L49,U236,R621,U656,R997,U268,L18,D789,L935,D87,L670,U35,R463,D71,R268,U728,R693,D863,R656,D654,L350,U796,L72,U562,R56,U10,L651,D751,L557,D518,R901,D741,R787,D332,R723,D980,R206,U670,R645,D927,L641,D863,R478,D568,L858,D990,L124,D864,L162,U361,L407,U674,R508,D284,L675,D794,L138,U55,L781,U37,R956,D364,L111,U721,L91,U559,L852,U351,R994,U446,L162,D345,R92,D941,R572,U185,R615,D590,R459,D313,R127,D315,R96,U751,R210,D620,L790,U826,R410,D652,R549,D698,L805,U814,L364,U905,L96,U997,L689\nL1008,D451,L146,D628,R877,U486,L464,U815,L119,U208,R686,U477,L510,D353,R189,D437,R461,D645,R639,U650,R491,D744,L798,U514,R598,U64,R668,U771,R21,U782,L564,U632,R23,U112,R947,U649,L205,D804,R277,U683,L828,U662,R890,U420,L908,U484,R535,D515,R390,U7,L287,D967,R497,U502,L893,D851,R426,D656,R622,U46,L106,U590,R646,D29,R467,D896,L155,U382,L992,D189,L34,U16,R132,U35,L586,U812,L539,D409,R776,D42,R58,U323,R569,D965,R648,D789,R478,D587,R162,D834,R979,D993,L944,U84,R93,U903,R491,U713,L646,U235,R120,U286,L919,U34,L662,U834,L812,D271,L73,U410,L758,U210,R712,U581,L520,D654,L981,D516,R312,U123,L153,U433,R368,U606,L882,U362,L261,U587,R441,D691,L699,U135,L825,D25,R142,U191,L358,D554,L487,D802,L542,D266,R283,U222,R113,D259,R828,U182,R402,U627,R769,D426,L768,U571,R118,U684,R803,D430,R942,U514,R711,D225,R299,U45,L214,U712,L673,U787,L164,D703,L616,D587,R624,D326,L614,D779,L904,D563,L98,U137,R687,U425,R615,U671,L361,D47,L767,D951,R791,D116,R664,U704,R291,U535,L322,D989,R467,U7,L974,D276,R901,U51,L567,D641,R112,U102,R753,D127,R486,D143,R259,U212,L97,U505,R377,U473,R514,D912,L928,U401,R772,D416,R695,U784,L524,D341,R402,U749,L1,U1,L109,U921,L754,U66,L927,U708,R551,D687,R129,D346,L408,D330,L300,D920,R170,D353,R97,D74,R850,D511,R275,U872,L748,U344,R610,D391,R963,D98,L89,U259,R651,U651,L31,D142,L104,U770,L482,D677,R823,D110,L606,U897,L631,U437,L551,D550,R301,D762,R349,D824,R260,U438,R249,D636,L386,U926,R367,U231,R752,U854,L481,D764,R516,D273,L726,D778,R483,U513,R129,D135,L224".split("\n"));

function distanceFromCenter(x, y) {
  return Pervasives.abs(x) + Pervasives.abs(y) | 0;
}

function key(x, y) {
  return String(Caml_int32.imul(x, 100000) + y | 0);
}

function addDirection(param, param$1) {
  return /* tuple */[
          param[0] + param$1[0] | 0,
          param[1] + param$1[1] | 0
        ];
}

function determineIntersection(state, param, wire) {
  var y = param[1];
  var x = param[0];
  var mapKey = key(x, y);
  var match = Belt_MapString.get(state[/* segments */1], mapKey);
  var match$1 = state[/* minIntersection */2];
  var exit = 0;
  if (match !== undefined && match === wire) {
    return /* record */[
            /* current : tuple */[
              x,
              y
            ],
            /* segments */state[/* segments */1],
            /* minIntersection */state[/* minIntersection */2]
          ];
  } else {
    exit = 1;
  }
  if (exit === 1) {
    var exit$1 = 0;
    if (match !== undefined) {
      var w = match;
      if (match$1) {
        if (distanceFromCenter(x, y) < match$1[1]) {
          var distance = distanceFromCenter(x, y);
          console.log(Curry._3(Printf.sprintf(/* Format */[
                        /* String_literal */Block.__(11, [
                            "New min distance ",
                            /* Int */Block.__(4, [
                                /* Int_d */0,
                                /* No_padding */0,
                                /* No_precision */0,
                                /* String_literal */Block.__(11, [
                                    " at ",
                                    /* Int */Block.__(4, [
                                        /* Int_d */0,
                                        /* No_padding */0,
                                        /* No_precision */0,
                                        /* Char_literal */Block.__(12, [
                                            /* "," */44,
                                            /* Int */Block.__(4, [
                                                /* Int_d */0,
                                                /* No_padding */0,
                                                /* No_precision */0,
                                                /* End_of_format */0
                                              ])
                                          ])
                                      ])
                                  ])
                              ])
                          ]),
                        "New min distance %d at %d,%d"
                      ]), distance, x, y));
          return /* record */[
                  /* current : tuple */[
                    x,
                    y
                  ],
                  /* segments */Belt_MapString.set(state[/* segments */1], mapKey, w),
                  /* minIntersection : Found */[
                    /* tuple */[
                      x,
                      y
                    ],
                    distance
                  ]
                ];
        } else {
          exit$1 = 2;
        }
      } else {
        var distance$1 = distanceFromCenter(x, y);
        return /* record */[
                /* current : tuple */[
                  x,
                  y
                ],
                /* segments */Belt_MapString.set(state[/* segments */1], mapKey, w),
                /* minIntersection : Found */[
                  /* tuple */[
                    x,
                    y
                  ],
                  distance$1
                ]
              ];
      }
    } else {
      exit$1 = 2;
    }
    if (exit$1 === 2) {
      return /* record */[
              /* current : tuple */[
                x,
                y
              ],
              /* segments */Belt_MapString.set(state[/* segments */1], mapKey, wire),
              /* minIntersection */state[/* minIntersection */2]
            ];
    }
    
  }
  
}

function updateMap(_state, direction, _amount, wire) {
  while(true) {
    var amount = _amount;
    var state = _state;
    var current = state[/* current */0];
    if (amount !== 0) {
      var y = direction[1];
      var x = direction[0];
      if (amount < 0) {
        var s = determineIntersection(state, addDirection(current, /* tuple */[
                  x,
                  y
                ]), wire);
        _amount = amount + 1 | 0;
        _state = s;
        continue ;
      } else if (amount > 0) {
        var s$1 = determineIntersection(state, addDirection(current, /* tuple */[
                  x,
                  y
                ]), wire);
        _amount = amount - 1 | 0;
        _state = s$1;
        continue ;
      } else {
        throw ParseException;
      }
    } else {
      return state;
    }
  };
}

function directionToNumbers(direction) {
  switch (direction) {
    case "D" : 
        return /* tuple */[
                0,
                -1
              ];
    case "L" : 
        return /* tuple */[
                -1,
                0
              ];
    case "R" : 
        return /* tuple */[
                1,
                0
              ];
    case "U" : 
        return /* tuple */[
                0,
                1
              ];
    default:
      throw ParseException;
  }
}

function processMovement(state, movement) {
  var match = new RegExp("([RDLU])(\\d+)").exec(movement);
  if (match !== null) {
    var match$1 = $$Array.map((function (prim) {
            if (prim == null) {
              return undefined;
            } else {
              return Caml_option.some(prim);
            }
          }), match);
    if (match$1.length !== 3) {
      throw ParseException;
    }
    var match$2 = match$1[1];
    if (match$2 !== undefined) {
      var match$3 = match$1[2];
      if (match$3 !== undefined) {
        var partial_arg = Caml_format.caml_int_of_string(match$3);
        var partial_arg$1 = directionToNumbers(match$2);
        return (function (param) {
            return updateMap(state, partial_arg$1, partial_arg, param);
          });
      } else {
        throw ParseException;
      }
    } else {
      throw ParseException;
    }
  } else {
    console.log("???");
    throw ParseException;
  }
}

if (input.length !== 2) {
  throw ParseException;
}

var wire1 = input[0];

var wire2 = input[1];

var s_000 = /* current : tuple */[
  0,
  0
];

var s = /* record */[
  s_000,
  /* segments */Belt_MapString.empty,
  /* minIntersection : NotFound */0
];

var s$1 = $$Array.fold_left((function (state, move) {
        return processMovement(state, move)(1);
      }), s, wire1);

var processWires = $$Array.fold_left((function (state, move) {
        return processMovement(state, move)(2);
      }), /* record */[
      /* current : tuple */[
        0,
        0
      ],
      /* segments */s$1[/* segments */1],
      /* minIntersection */s$1[/* minIntersection */2]
    ], wire2);

var match = processWires[/* minIntersection */2];

if (match) {
  var match$1 = match[0];
  console.log(Curry._3(Printf.sprintf(/* Format */[
                /* String_literal */Block.__(11, [
                    "Part 1: ",
                    /* Int */Block.__(4, [
                        /* Int_d */0,
                        /* No_padding */0,
                        /* No_precision */0,
                        /* String_literal */Block.__(11, [
                            " at ",
                            /* Int */Block.__(4, [
                                /* Int_d */0,
                                /* No_padding */0,
                                /* No_precision */0,
                                /* Char_literal */Block.__(12, [
                                    /* "," */44,
                                    /* Int */Block.__(4, [
                                        /* Int_d */0,
                                        /* No_padding */0,
                                        /* No_precision */0,
                                        /* End_of_format */0
                                      ])
                                  ])
                              ])
                          ])
                      ])
                  ]),
                "Part 1: %d at %d,%d"
              ]), match[1], match$1[0], match$1[1]));
} else {
  console.log("Not found!");
}

exports.ParseException = ParseException;
exports.input = input;
exports.distanceFromCenter = distanceFromCenter;
exports.key = key;
exports.addDirection = addDirection;
exports.determineIntersection = determineIntersection;
exports.updateMap = updateMap;
exports.directionToNumbers = directionToNumbers;
exports.processMovement = processMovement;
exports.processWires = processWires;
/* input Not a pure module */
