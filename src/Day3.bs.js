...
var ParseException = Caml_exceptions.create("Day3-AdventOfCode2019.ParseException");

var InvalidWireException = Caml_exceptions.create("Day3-AdventOfCode2019.InvalidWireException");

var input = $$Array.map((function (param) {
        return param.split(",");
...
function intToWire(wire) {
  if (wire !== 1) {
    if (wire !== 2) {
      throw InvalidWireException;
    } else {
      return /* Wire2 */1;
    }
  } else {
...
function getSteps(move, wire) {
  if (wire) {
    return Caml_array.caml_array_get(move[/* steps */1], 1);
  } else {
    return Caml_array.caml_array_get(move[/* steps */1], 0);
  }
}

function setSteps(move, wire, value) {
  if (wire) {
    Caml_array.caml_array_set(move[/* steps */1], 1, value);
  } else {
    Caml_array.caml_array_set(move[/* steps */1], 0, value);
  }
  return move;
}
...
function key(x, y) {
  return String(Caml_int32.imul(x, 100000) + y | 0);
}

function addDirection(param, param$1) {
  return /* tuple */[
          param[0] + param$1[0] | 0,
          param[1] + param$1[1] | 0
        ];
...
function determineIntersection(state, position, wire) {
  var y = position[1];
  var x = position[0];
  var state_000 = /* current */state[/* current */0];
  var state_001 = /* stepCount */state[/* stepCount */1] + 1 | 0;
  var state_002 = /* segments */state[/* segments */2];
  var state_003 = /* minIntersection */state[/* minIntersection */3];
  var state_004 = /* minSteps */state[/* minSteps */4];
  var mapKey = key(x, y);
  var partial_arg = state_002;
  var setSegment = function (param) {
    return Belt_MapString.set(partial_arg, mapKey, param);
  };
  var distance = distanceFromCenter(x, y);
  var sumSteps = function (m) {
    return Caml_array.caml_array_get(m[/* steps */1], 0) + Caml_array.caml_array_get(m[/* steps */1], 1) | 0;
  };
  var updateMinSteps = function (minSteps, steps) {
    if (minSteps) {
      return /* Found */[Caml_primitive.caml_int_min(steps, minSteps[0])];
    } else {
      return /* Found */[steps];
    }
  };
  var updateMinIntersection = function (move, minSteps) {
    var steps = sumSteps(move);
    return /* record */[
            /* current */position,
            state_001,
            /* segments */Curry._1(setSegment, move),
            /* minIntersection : Found */[
              position,
              distance
            ],
  ...
  var match = Belt_MapString.get(state_002, mapKey);
  var match$1 = state_003;
  var match$2 = state_004;
  if (match !== undefined) {
    if (match[/* wire */0] === wire) {
      return /* record */[
              /* current : tuple */[
                x,
                y
      ...
      var move = match;
      if (match$1) {
        if (distance < match$1[1]) {
          var move$1 = setSteps(move, wire, state_001);
          return updateMinIntersection(move$1, match$2);
        } else {
          var move$2 = setSteps(move, wire, state_001);
          var steps = sumSteps(move$2);
          return /* record */[
                  /* current */position,
                  state_001,
                  /* segments */Curry._1(setSegment, move$2),
                  state_003,
                  /* minSteps */updateMinSteps(match$2, steps)
                ];
        }
      } else {
        var move$3 = setSteps(move, wire, state_001);
        return updateMinIntersection(move$3, match$2);
      }
    }
  } else {
    var move$4 = setSteps(/* record */[
          /* wire */wire,
          /* steps */Caml_array.caml_make_vect(2, 0)
        ], wire, state_001);
    return /* record */[
            /* current : tuple */[
              x,
            ...
            state_001,
            /* segments */Curry._1(setSegment, move$4),
            state_003,
            state_004
          ];
  }
...
function updateMap(_state, direction, _amount, wire) {
  while(true) {
    var amount = _amount;
    var state = _state;
    ...
    if (amount !== 0) {
      ...
      var x = direction[0];
      if (amount < 0) {
        var s = determineIntersection(state, addDirection(current, /* tuple */[
                  x,
                  y
                ]), wire);
        _amount = amount + 1 | 0;
        ...
        var s$1 = determineIntersection(state, addDirection(current, /* tuple */[
                  x,
                  y
                ]), wire);
        _amount = amount - 1 | 0;
...
function directionToNumbers(direction) {
  switch (direction) {
    case "D" :
        return /* tuple */[
                0,
...
function processMovement(state, movement, wire) {
  var match = new RegExp("([RDLU])(\\d+)").exec(movement);
  if (match !== null) {
    ...
    if (match$2 !== undefined) {
      ...
      if (match$3 !== undefined) {
        return updateMap(state, directionToNumbers(match$2), Caml_format.caml_int_of_string(match$3), wire);
      } else {
        throw ParseException;
      }
    } else {
...
var s = /* record */[
  s_000,
  /* stepCount */0,
  /* segments */Belt_MapString.empty,
  /* minIntersection : NotFound */0,
  /* minSteps : NotFound */0
];

var s$1 = $$Array.fold_left((function (state, move) {
        return processMovement(state, move, /* Wire1 */0);
      }), s, wire1);

var processWires = $$Array.fold_left((function (state, move) {
        return processMovement(state, move, /* Wire2 */1);
      }), /* record */[
      /* current : tuple */[
        0,
        0
      ...
      /* stepCount */0,
      /* segments */s$1[/* segments */2],
      /* minIntersection */s$1[/* minIntersection */3],
      /* minSteps */s$1[/* minSteps */4]
    ], wire2);

var match = processWires[/* minIntersection */3];
...
var match$2 = processWires[/* minSteps */4];
...
exports.InvalidWireException = InvalidWireException;
exports.input = input;
exports.wireCount = wireCount;
exports.intToWire = intToWire;
exports.getSteps = getSteps;
exports.setSteps = setSteps;
exports.distanceFromCenter = distanceFromCenter;
exports.key = key;
exports.addDirection = addDirection;
exports.determineIntersection = determineIntersection;
exports.updateMap = updateMap;
exports.directionToNumbers = directionToNumbers;
exports.processMovement = processMovement;
exports.processWires = processWires;
